...
public:
  ...
  ImpitWrapper(const Napi::CallbackInfo& info) : Napi::ObjectWrap<ImpitWrapper>(info) {
    ...
    if (info.Length() >= 1 && info[0].IsObject()) {
      ...
      if (o.Has("ignoreTlsErrors")) verify = !o.Get("ignoreTlsErrors").As<Napi::Boolean>().Value();
      if (o.Has("caPath")) caPath = o.Get("caPath").As<Napi::String>().Utf8Value();
      if (o.Has("followRedirects")) followRedirects = o.Get("followRedirects").As<Napi::Boolean>().Value();
      if (o.Has("headers") && o.Get("headers").IsObject()) {
        Napi::Object h = o.Get("headers").As<Napi::Object>();
        auto props = h.GetPropertyNames();
        for (uint32_t i=0;i<props.Length();++i) {
